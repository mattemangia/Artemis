...
/// <summary>
/// Enhanced renderer with support for joints, raycasts, and triggers
/// </summary>
public class AdvancedRenderer
{
    ...
    public AdvancedRenderer(int width, int height, float scale = 2f)
    {
        _width = width;
        _height = height;
        _scale = scale;
    ...
    public void DrawCircle(Vector2 center, float radius, char c, ConsoleColor color, bool filled = true)
    {
        ...
        for (int dy = -screenRadius; dy <= screenRadius; dy++)
        {
            for (int dx = -screenRadius; dx <= screenRadius; dx++)
            {
                ...
                if (filled && dist <= screenRadius)
                {
                    int x = (int)screenPos.X + dx;
                    int y = (int)screenPos.Y + dy;
                    SetPixel(x, y, c, color);
                }
                else if (!filled && dist <= screenRadius && dist >= screenRadius - 1)
                {
                    int x = (int)screenPos.X + dx;
                    int y = (int)screenPos.Y + dy;
                    SetPixel(x, y, c, color);
    ...
    public void DrawBox(Vector2 center, float width, float height, char c, ConsoleColor color, bool filled = true)
    {
        ...
        else
        {
            ...
            for (int dx = 0; dx < screenWidth; dx++)
            {
                SetPixel(startX + dx, startY, c, color);
                SetPixel(startX + dx, startY + screenHeight - 1, c, color);
            }
            for (int dy = 0; dy < screenHeight; dy++)
            {
                SetPixel(startX, startY + dy, c, color);
                SetPixel(startX + screenWidth - 1, startY + dy, c, color);
            }
        }
    }

    public void DrawRaycast(Ray ray, RaycastHit hit)
    {
        Vector2 endpoint = hit.Hit ? hit.Point : ray.GetPoint(ray.MaxDistance);

        DrawLine(ray.Origin, endpoint, hit.Hit ? 'x' : '.',
                 hit.Hit ? ConsoleColor.Red : ConsoleColor.DarkGray);

        if (hit.Hit)
        {
            // Draw hit point
            DrawCircle(hit.Point, 0.2f, 'X', ConsoleColor.Yellow);
        }
    }
